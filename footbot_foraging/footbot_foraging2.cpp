...
void CFootBotForaging::Explore() { 
   ...
   if(bReturnToNest) {
	 ...
	 CVector2 Coordinate;
	 float X[tPackets.size()];
	 float Y[tPackets.size()];
	 int count = 0;
      ...
      // if any packets are found chose one vector from array and switch to STATE::TO_TARGET 
         if (FoundTarget == true){
           CRange range = -(count-1);
           int nr = round((count-1) + m_pcRNG->Uniform(range));
           LOGERR<<count<<" | "<<nr<<" | "<<X[0]<<" | "<<X[1]<<" | "<<X[2]<<" | "<<X[3]<<" | "<<X[4]<<std::endl;
      	   FoodPos = CVector2(X[nr],Y[nr]);
